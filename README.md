...
The project uses a custom CLI (`@rspack-mf/cli`) to generate Rspack configurations. The configuration supports three versions of module federation:
...
// Module Federation Configuration
{
  ...
  exposes: {
    './block': './path/to/block.tsx'
  },
  shared: {
    'react': {
      singleton: true,
      requiredVersion: '*', // for remotes
      // or specific version for host
      requiredVersion: normalizeVersion(pkg.version)
    },
    'react-dom': { /* similar config */ },
    '@rspack-mf/utils': { /* similar config */ }
  }
}
```
